...
const Sidebar: React.FC = () => {
	...
	const {
		...
		searchNoResults,
		searchUpdate,
	} = useSelector((state: StoreState) => state.search);
	...
	useEffect(() => {
		...
		switch (searchType) {
			...
			case "QUERY_PREVISAO_POR_PARADA":
				api.get(`/PrevisaoPorParada?CodigoParada=${searchValue}`)
					.then((res) => {
		...
		<>
			...
			>
				<Paper className="paper" component="form">
					...
					<InputBase
						...
						id="outlined-basic"
						placeholder={
							value === 0 ? "Buscar Linha" : "Buscar Parada"
						}
					/>
					<IconButton
			...
			<div role="tabpanel" hidden={value !== 0} id={`${value}`}>
				<div
					...
					style={{
						display:
							linhas.length > 0 || searchNoResults
								? "none"
								: "flex",
					}}
				>
					<p>
						A categoria Linhas possibilita a consulta pelas linhas
						de ônibus da cidade de São Paulo.
					</p>
					<p>
						Você pode buscar pelo número da linha ou por algum termo
						relacionada a linha.
					</p>
				...
				<div
					...
					style={{ display: searchNoResults ? "flex" : "none" }}
				>
					<Warning />
				...
				{linhas.map((linhas: LinhasTypes) => (
					<div className="linhaContainer" key={linhas.cl}>
						<Paper
							className="linhaCard"
							onClick={(e) => handleLoadCarByLinhas(e, linhas)}
						>
							<div className="codigoLinha">
								<p className="codLinha">LINHA</p>
								<h3
									className="nomeLinha"
									aria-label="Codigo da Linha"
								>
									{linhas.cl}
								</h3>
							</div>
							<div className="infoParadas">
								<div className="detalhesParada">
									...
									<h4 className="enderecoParada">
										{linhas.sl ? linhas.tp : linhas.ts}
									</h4>
								</div>
								<IconButton className="viewLinhas">
									<ArrowRightAlt />
								</IconButton>
								<div className="detalhesParada">
									<p className="direcaoParada">DESTINO</p>
									<h4 className="enderecoParada">
										{linhas.sl ? linhas.ts : linhas.tp}
									</h4>
								</div>
							</div>
							<IconButton className="viewLinhas">
			...
			<div
				role="tabpanel"
				hidden={value !== 1 || paradasDetalhes}
				id={`${value}`}
			>
				<div
					className="noSearch"
					style={{
						display:
							paradas.length > 0 || searchNoResults
								? "none"
								: "flex",
					}}
				>
					<p>
						A categoria Paradas possibilita a consulta pelos pontos
						de parada da cidade de São Paulo. Atualmente esta
						categoria contempla apenas as paradas de corredores.
					</p>
					<p>
						Você pode buscar pelo nome da parada e também no seu
				...
				{paradas.map((parada: ParadasTypes) => (
					<div className="paradaContainer" key={parada.cp}>
						<Paper
							className="paradaCard"
							onClick={() => handleLoadParadas(parada)}
						>
			...
			{paradasDetalhes && (
				<div className="flip-horizontal-bottom previsõesParada">
					<IconButton
						className="closePrevisoes"
						onClick={() => setParadasDetalhes(false)}
					>
						<Cancel />
					</IconButton>
					{posicoes.length > 0 &&
							...
							<Paper key={dataLinhas.c} className="previsaoCard">
								<div className="infoLinha">
									...
									<p className="label">
										Veiculos
										<br />
										<strong className="infoVeiculos">
											{dataLinhas.qv}
										</strong>
									</p>
								</div>
								<div className="direcaoPrevisao">
									<p className="enderecoPrevisao">
										origen:{" "}
										<strong className="local">
											{dataLinhas.sl
												? dataLinhas.lt1
												: dataLinhas.lt0}
										</strong>
								...
								<div className="preivsaoLinha">
									{dataLinhas.vs.map((bus: BusGeo) => (
										<div
											key={bus.p}
											className="previsoes"
											onClick={() => handleNewCenter(bus)}
										>
											<div className="onibus">
												<p className="onibusNome">
													ônibus
												</p>
												<h3 className="codOnibus">
													{bus.p}
												</h3>
											</div>
											<Divider
											...
											<div className="chegada">
												<p className="labelPrevisao">
													Previsão de chegada
												</p>
												<h3 className="horaPrevisao">
													{bus.t}
												</h3>
											</div>
											<IconButton className="viewLinhas">
			...
			<div role="tabpanel" hidden={value !== 2} id={`${value}`}>
				<div>
					<Paper className="overview">
						<p className="infoOverview">
							O modo Overview permite visualizar todos os ônibus
							de São Paulo
						</p>
						<small className="infoAviso">
							Devido ao grande numero de veiculos, o modo Overview
							pode causar travamentos e redução de desenpenho
						</small>
						<FormControlLabel
				...
				<div className="opcoes">
					<Paper>
						<FormControlLabel
							...
							onChange={handleSearchUpdadeState}
							control={<Switch />}
							label={
								searchUpdate
					...
					<Paper className="infoProjeto">
						...
						<div className="dados">
							<img
								src="./SPTrans_logo.png"
								alt="Logo SpTrans"
							></img>
							<h1 className="descritivo">
								Todos os dados são obtidos através da API DO
								OLHO VIVO da SPTrans{" "}
							</h1>
						</div>
					</Paper>
					<Paper className="infoProjeto">
						<a
							href="https://github.com/renan905/programa-estagio"
							target="blank"
						>
							<GitHub />
						</a>
						<h1 className="descritivo">
							Todo o codigo deste projeto esta disponível no
							github
						</h1>
					</Paper>

					<Paper className="infoProjeto">
						<h2 className="descritivo">
							São Paulo Bus Traker foi desenvolvidor por
						</h2>
