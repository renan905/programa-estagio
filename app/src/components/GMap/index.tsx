...
const GMap: React.FC = () => {
	...
	const { searchValue, searchType, searchUpdate } = useSelector(
	...
	const [update, setUpdate] = useState(() => {
		return {
			state: false,
			time: searchType === "QUERY_DISPLAY_ALL" ? 60000 : 30000,
		};
	});
	useEffect(() => {
		if (searchUpdate) {
	...
	// MAP CONFIG
	const { center, mapStyle, zoom } = useSelector(
		(state: StoreState) => state.mapconfig
	);
	...
	useEffect(() => {
		...
		switch (searchType) {
				...
				api.get("/Posicao")
					.then((res) => {
						...
						if (res.data.l.length <= 0) {
							...
							handleAgoraNoMapa(
								"OVERVIEW - Todos os ônibus de São Paulo"
							);
						}
					})
					.catch((err) => {
			...
			case "QUERY_POSICAO_POR_LINHA":
				api.get(`/PosicaoPorLinha?CodigoLinha=${searchValue}`)
					.then((res) => {
						handleMetaData(true, res.data.hr);
						if (res.data.vs.length <= 0) {
							...
							handleAgoraNoMapa(
								`Todos os ônibus da linha ${searchValue}`
							);
							dispatch(
								mapCenterZoom({
									center: {
				...
				api.get(`/ParadasPorLinha?CodigoLinha=${searchValue}`)
					.then((res) => {
						setParadas(res.data);
					})
					.catch((err) => {
				...
				api.get(`/PrevisaoPorParada?CodigoParada=${searchValue}`)
					.then((res) => {
						...
						if (res.data.p.length <= 0) {
							...
							handleAgoraNoMapa(
								`Todos os ônibus em direção a ${searchValue}`
							);
						}
					})
					.catch((err) => {
	...
	const [infoWindowsSelectParadas, setInfoWindowsParadas] = useState(() => {
		const paradasMarker: ParadasTypes = {
			cp: 0,
			np: "",
			ed: "",
			px: 0,
	...
	const handleSelectInfoWindowParada = (
		parada: ParadasTypes,
		state: boolean = false
	) => {
		...
		<div className="mapContainer">
			...
			<LoadScript googleMapsApiKey={process.env.REACT_APP_GOOGLE_API}>
				...
				>
					{searchType === "QUERY_DISPLAY_ALL" &&
						posicoes.map((busLinhas: BusLinhas) =>
							busLinhas.vs.map((bus: BusGeo) => (
								<Marker
									onLoad={() =>
										handleMetaData(false, "", true, false)
									}
									key={bus.p}
					...
					{searchType === "QUERY_POSICAO_POR_LINHA" &&
						posicoes.map((bus: BusGeo) => (
							<Marker
								...
								position={{ lat: bus.py, lng: bus.px }}
								onClick={() =>
									handleSelectInfoWindowBus(bus, true)
								}
								icon={{
									url: "/bus1.svg",
					...
					;
					{posicoes.length > 0 &&
						searchType === "QUERY_PREVISAO_POR_PARADA" &&
						posicoes.map((busLinhas: BusLinhas) =>
							busLinhas.vs.map((bus: BusGeo) => (
								<Marker
									onLoad={() =>
					...
					{paradas.length > 0 &&
						...
						paradas.map((parada: ParadasTypes) => (
							<Marker
								onLoad={() =>
									handleMetaData(false, "", false, true)
								}
								key={parada.cp}
								position={{ lat: parada.py, lng: parada.px }}
								onClick={() =>
									handleSelectInfoWindowParada(parada, true)
								}
								icon={{
									url: "/bus-stop2.svg",
					...
					;
					{(searchType === "QUERY_BUSCAR_PARADA" ||
						searchType === "QUERY_PREVISAO_POR_PARADA") &&
						paradas.map((parada: ParadasTypes) => (
							<Marker
								visible={
									searchType === "QUERY_BUSCAR_PARADA"
										? true
										: parada.cp.toString() === searchValue
										? true
										: false
								}
								onLoad={() =>
					...
					{infoWindowsSelectBus.select ? (
						<InfoWindow
							position={{
								lat: infoWindowsSelectBus.busMarker.py,
								lng: infoWindowsSelectBus.busMarker.px,
							}}
							onCloseClick={() =>
								handleSelectInfoWindowBus(
									infoWindowsSelectBus.busMarker,
									false
								)
							}
						>
							<div className="busInfoBox">
								<h1>
									<AirportShuttle /> Prefixo do veículo:{" "}
									<strong className="status">
										{infoWindowsSelectBus.busMarker.p}
									</strong>
								</h1>
								<h1>
									<Accessible /> Suporte à Acessibilidade:{" "}
									<strong className="status">
										{infoWindowsSelectBus.busMarker.a
											? "SIM"
											: "NÃO"}
									</strong>
								</h1>
								<h1>
									<AccessTime /> Atualizado as :{" "}
									<strong className="status">
										{handleIsoTimeParse(
											infoWindowsSelectBus.busMarker.ta
										)}
									</strong>
								</h1>
							</div>
					...
					{infoWindowsSelectParadas.select ? (
						<InfoWindow
							position={{
								lat: infoWindowsSelectParadas.paradasMarker.py,
								lng: infoWindowsSelectParadas.paradasMarker.px,
							}}
							onCloseClick={() =>
								handleSelectInfoWindowParada(
									infoWindowsSelectParadas.paradasMarker,
									false
								)
							}
						>
